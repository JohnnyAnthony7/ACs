...
def potencia_de_dois_veio_sem_exp():
        return jsonify({"erro":"sem expoente"}), 404
...
Alias, lembre no exemplo: as query strings nao precisam ser colocadas
no app.route. Elas aparecem de outra forma, em um dicionário.
'''
@app.route("/ola_upgrade")
def ola2():
        dici=request.args
        if not 'pessoa1' in dici:
                p1=None
        else:
                p1=dici['pessoa1']
        if not 'pessoa2' in dici:
                p2=None
        else:
                p2=dici['pessoa2']
                
        if p1==None or p2==None:
                return jsonify({'erro':'argumentos faltando'}),404
        return 'ola %s e %s' % (p1, p2)
'''
Exercicio
...
def ola4():
        ...
        return jsonify({'pessoa1': p1, 'pessoa2':p2})
'''
Exercicio
